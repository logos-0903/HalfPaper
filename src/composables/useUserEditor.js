...
export function useUserEditor() {
  ...
  function hydrateFormFromProfile(currentProfile) {
    ...
    form.school = currentProfile?.school || ''
  ...
  async function saveProfile() {
    ...
    const stableEmail = String(profile.value?.email || '').trim()
    ...
    const payload = {
      ...
      sign: form.sign.trim() || undefined,
      ...(uploadedAvatarFilename.value ? { avatar: uploadedAvatarFilename.value } : {})
    }

    if (stableEmail) {
      payload.email = stableEmail
    }
    ...
    try {
      ...
      try {
        serverProfile = await updateUserProfile(payload)
      } catch (error) {
      ...
      const mergedProfile = {
        ...
        email: stableEmail || profile.value?.email || '',
      ...
      if (serverProfile && typeof serverProfile === 'object') {
        Object.assign(mergedProfile, serverProfile)
        if (serverProfile.avatar) {
          mergedProfile.avatar = resolveAvatarUrl(serverProfile.avatar)
        }
      }
